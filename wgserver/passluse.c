/*++   Copyright (c) 1995, 1996  Microsoft Corporation   Module Name:
       pswdntfy.c   Abstract:
       ...
       password quality.   Author:       Scott Field (sfield)    14-May-96
   --*/   
#include <windows.h>
...
NTSTATUS   NTAPI   PasswordChangeNotify(
       ...
       PUNICODE_STRING Password       )   /*++   Routine Description:
       This (optional) routine is notified of a password change.   Arguments:
       UserName - Name of user whose password changed
       RelativeId - RID of the user whose password changed
       NewPassword - Cleartext new password for the user   Return Value:
       STATUS_SUCCESS only - errors from packages are ignored.   --*/   
{
#ifdef DEBUG       WCHAR String[ 256 ];       
	   swprintf(String, L"Password for account %.*ls (rid 0x%x) changed to %.*ls\n",
           UserName->Length / sizeof(WCHAR), UserName->Buffer,
           RelativeId, Password->Length / sizeof(WCHAR),
           Password->Buffer);
...
BOOL   NTAPI   PasswordFilter(       PUNICODE_STRING UserName,
        ...
        on the specified account will fail.   --*/   
{
	   ...
	   if(GetStringTypeW(
           CT_CTYPE1, Password->Buffer, cchPassword,
           CharType)) {
...
BOOL   NTAPI   InitializeChangeNotify(void)   /*++
   Routine Description:
       ...
       is loaded.   Arguments:   Return Value:
...
{   
       ...
       // events, etc.       //       return TRUE;   }   
	   /********
   pswdntfy.def   ------------   LIBRARY pswdntfy   EXPORTS
